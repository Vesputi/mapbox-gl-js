...
function evaluateSizeForZoom(sizeData: SizeData, currentZoom: number, property: Property<number, PossiblyEvaluatedPropertyValue<number>>) {
    if (sizeData.functionType === 'constant') {
        ...
        const t = util.clamp(
            expression.interpolationFactor(zoomRange.min, zoomRange.max, currentZoom),
            0, 1);
        ...
        return {
            uSizeT: util.clamp(expression.interpolationFactor(zoomRange.min, zoomRange.max, currentZoom), 0, 1),
            uSize: 0
        };
    }
}
